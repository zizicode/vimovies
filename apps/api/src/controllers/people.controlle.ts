...
export const PeopleController = {
  ...
  async getBySlug(c: Context) {
    try {
      ...
      const result   = await PeopleService.findBySlugWithFilmography(slug ?? '')
  ...
  async search(c: Context) {
    try {
      ...
      const results = await PeopleService.search(q, limit)
      return ok(c, results)
    } catch (err) {
      return serverError(c, err)
    }
  ...
  async adminList(c: Context) {
    try {
      ...
      const { data, total } = await PeopleService.findAll(page, per_page)
